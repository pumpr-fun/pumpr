...
function enhanceTerminalAccessibility(dom, state) {
  ...
  terminal.setAttribute("aria-atomic", "false");
  terminal.setAttribute("aria-label", "Airi live terminal output");
  ...
  terminal.style.lineHeight = "1.75em";
  terminal.style.fontFamily = "Consolas, 'Courier New', monospace";
  terminal.style.fontSize = "15px";

  // Focus and blur outlines for keyboard users
  terminal.addEventListener("focus", () => {
    terminal.style.outline = "3px solid #67f2aa";
    terminal.style.outlineOffset = "4px";
  });
  ...
  terminal.setAttribute("title", "Terminal output. Use arrow keys, Page Up/Down, Home/End, and Space to scroll.");
  ...
  terminal.setAttribute("aria-describedby", instructionsId);
}
...
function boot() {
  ...
  // Enhance terminal accessibility and keyboard navigation
  enhanceTerminalAccessibility(dom, state);
